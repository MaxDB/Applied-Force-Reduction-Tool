classdef Dynamic_Solution
    ...
    methods
        ...
        % Analysis 
        %-----------------------------------------------------------------%
        function Analysis_Output = analyse_solution(~,solution_num,Add_Output,extra_output)
            solution_name = "temp\dynamic_sol_" + solution_num; 
            bd = coco_bd_read(solution_name);
            
            output_string = {"po.period", "LAB", "TYPE", "eigs","ENERGY","MAX(x)","MIN(x)"};
            is_extra_output = (exist("extra_output","var") && ~isempty(extra_output));
            if is_extra_output
                output_string{end+1} = extra_output;
            end

            solution_data  = coco_bd_col(bd, output_string);
            ...
            switch Add_Output.output
                case "physical displacement"
                 ...
                 %extra output
            if is_extra_output
                Analysis_Output.extra = solution_data{1,end};
            end
        end
        %-----------------------------------------------------------------%
