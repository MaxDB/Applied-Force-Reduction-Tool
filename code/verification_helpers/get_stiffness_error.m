...
stiffness_error = zeros(num_points,num_test_cases);
...
for iDisp = 1:num_points
    ...
    for iTest = 1:num_test_cases
        ...
        stiffness_error(iDisp,iTest) = abs(energy_one - energy_two)/abs(energy_one);
    end
end

stiffness_error = max(stiffness_error,[],2);
end