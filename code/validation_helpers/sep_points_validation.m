...
for iIteration = 1:(max_iterations+1)
    ...
    for iSep = 1:num_validated_seps %parallelise 
        ...
        disp_one = evaluate_polynomial(Disp_Poly_One,sep_r);
        disp_two = evaluate_polynomial(Disp_Poly_Two,sep_r);
        disp_error = discrepency_error(disp_one,disp_two);
        [max_disp_error,max_error_index] = max(disp_error,[],1);
        norm_disp_error = max_disp_error/max_interpolation_error(2);
